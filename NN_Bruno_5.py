...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 14 15:31:56 2021
...
input_bruno = np.concatenate((set1, set2, set3), axis=1)
...
target = (input_bruno[:, 0] + input_bruno[:, 1] + input_bruno[:, 2]).reshape(10,1)
...
dim3_min, dim3_max = input_bruno[:,2].min(), input_bruno[:,2].max()
...
error = nn.train(input_bruno, target, show=15, goal=0.00001)
...
#Calculate the target variable based on the three inputs
target = (input_bruno[:, 0] + input_bruno[:, 1] + input_bruno[:, 2]).reshape(100,1)
